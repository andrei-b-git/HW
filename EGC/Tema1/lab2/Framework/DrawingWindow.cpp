...
bool* DrawingWindow::keyStates = new bool[256];
bool* DrawingWindow::keySpecialStates = new bool[256];
...
DrawingWindow::DrawingWindow(int argc, char** argv,int _width, int _height, int startX, int startY, string name)
{
	...
	glutKeyboardUpFunc(keyboardFunctionUp);	//functia apelata la eliberarea unei taste
	glutSpecialFunc(specialFunction); //functia apelata la apasarea unei taste speciale
	glutSpecialUpFunc(specialFunctionUp);	//functia apelata la eliberarea unei taste speciale
...
void DrawingWindow::displayFunction()
{
	...
	// Se deseneaza imaginea de fundal
...
void DrawingWindow::keyboardFunctionUp(unsigned char key, int x, int y)
{
	keyStates[key] = false;
	onKey(key);
}
